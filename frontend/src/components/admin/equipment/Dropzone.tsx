...
interface DropzoneProps {
    equipmentPicURL?: string; // Optional prop for image URL
    onDrop: (files: File[]) => void;
}

const Dropzone: React.FC<DropzoneProps> = ({ equipmentPicURL, onDrop }) => {
    ...
    return (
        ...
        >
            <input {...getInputProps()} />
            {equipmentPicURL || files.length > 0 ? (
                <div style={{ width: "100%", height: "100%", position: "relative" }}>
                    <img
                        src={equipmentPicURL || files[0].preview}
                        alt="Preview"
                        style={{
                            width: "100%",
                            height: "100%",
