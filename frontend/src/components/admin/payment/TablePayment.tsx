...
import { GetPayments } from "../../../service/https/payment";
...
const TablePayment: React.FC = () => {
  ...
  useEffect(() => {
    const getPayments = async () => {
      let res = await GetPayments(); // Assuming GetPayments is an API call
      if (res) {
        setPayments(res);
      }
    };
      ...
      <table className="w-[1000px] text-lg text-left text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-black uppercase bg-gray-50 dark:bg-green dark:text-black">
          <tr>
            ...
            <th scope="col" className="px-6 py-3 text-xl ">Payment Method</th>
            <th scope="col" className="px-6 py-3 text-xl ">Amount</th>
            <th scope="col" className="px-6 py-3 text-xl ">Member ID</th>
            <th scope="col" className="px-6 py-3 text-xl ">Package ID</th>
